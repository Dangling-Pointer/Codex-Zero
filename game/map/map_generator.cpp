...
MapGenerator::MapGenerator(const elysia::core::Vector2 gridDimensions, MapConfig config, TileMap &grid)
    : _gridDimensions{
          static_cast<float>(std::max(1, static_cast<int>(std::floor(gridDimensions.x)))),
          static_cast<float>(std::max(1, static_cast<int>(std::floor(gridDimensions.y))))},
      grid(grid),
      config(config)
{
    config.baseRoomSize.x = std::clamp(config.baseRoomSize.x, 1.0f, _gridDimensions.x);
    config.baseRoomSize.y = std::clamp(config.baseRoomSize.y, 1.0f, _gridDimensions.y);
    config.minSubRoomSize.x = std::clamp(config.minSubRoomSize.x, 1.0f, _gridDimensions.x);
    config.minSubRoomSize.y = std::clamp(config.minSubRoomSize.y, 1.0f, _gridDimensions.y);
    config.numSubRooms = std::max(0, config.numSubRooms);
    config.minGap = std::max(0, config.minGap);
    this->config = config;
...
void MapGenerator::generateRoom()
{
    ...
    elysia::core::Rect base = buildBaseRoom();
    addRecToGrid(base);

    for (int i = 0; i < config.numSubRooms; i++)
    {
        elysia::core::Rect subRoom = buildSubRoom(base);
...
elysia::core::Rect MapGenerator::buildBaseRoom()
{
    elysia::core::Rect room{0.0f, 0.0f, config.baseRoomSize.x, config.baseRoomSize.y};
    elysia::core::Vector2 center = {_gridDimensions.x / 2.0f, _gridDimensions.y / 2.0f};
...
elysia::core::Rect MapGenerator::buildSubRoom(elysia::core::Rect &base, Side side)
{
    if (side == Side::None)
    {
...
void MapGenerator::addRecToGrid(elysia::core::Rect &rec)
{
    auto &tiles = grid.data();
    const int width = static_cast<int>(_gridDimensions.x);
    const int height = static_cast<int>(_gridDimensions.y);
    const int left = std::clamp(static_cast<int>(rec.x()), 0, width);
    const int top = std::clamp(static_cast<int>(rec.y()), 0, height);
    const int right = std::clamp(static_cast<int>(rec.x() + rec.width()), 0, width);
    const int bottom = std::clamp(static_cast<int>(rec.y() + rec.height()), 0, height);
